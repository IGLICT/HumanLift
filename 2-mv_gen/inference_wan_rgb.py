...
test_list_path= [
    ...
    # [1, "data/images/002_00026_0061.png", "data/driven_poses/002_00026_0061"],
    ...
    [1, "data/images/b.png", "data/driven_poses/b"],
]
...
for img_file in sorted(os.listdir(image_dir)):
    if img_file.lower().endswith('.png') or img_file.lower().endswith('.jpg') or img_file.lower().endswith('.jpeg'):
        ...
        # Special case for trump_processed
        if "_processed" in base_name:
            # 对于包含"_processed"的文件，使用文件名前半部分作为目录名
            dir_name = base_name.split("_processed")[0]
            pose_dir = f"{pose_base_dir}/{dir_name}/temp/smplx1/{typea}"
        else:
            # 普通文件直接使用文件名作为目录名
            # base_name = '10'
            pose_dir = f"{pose_base_dir}/{base_name}/temp/smplx1/{typea}"

        if not os.path.exists(os.path.join(pose_dir, "pose.jpg")):
            print(f"Pose directory does not exist: {pose_dir}")
...
# rgb
chpt_folder = 'models_out10_1'
version_id = 'version_2'
ckpt_id = 'epoch=7-step=12000.ckpt' # 5 1 3000, 7 1 3000, 8 7 12000

save_dir = "./outputs0826-1" + \
...
for path_dir_per in test_list_path:
    ...
    save_video(video_out, save_dir + "/video_480P_{}.mp4".format(ref_image_path.split('/')[-1].split('.')[0]), fps=15, quality=5,save_frames_dir=None) #os.path.join(save_dir, "frames_480P_{}_{}".format(ref_image_path.split('/')[-1], pose_file_path.split('/')[-1])) ) # 720P
