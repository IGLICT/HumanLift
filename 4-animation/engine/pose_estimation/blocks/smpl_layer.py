...
class SMPL_Layer(nn.Module):
    ...
    def forward_zbt(
        ...
        transl = (transl.unsqueeze(1) + person_center + pelvis).squeeze(1)
        transl_up = transl.clone()
        # Moving into the camera coordinate system
        j3d_cam = j3d + transl_up.unsqueeze(1)
        verts_cam = verts + transl_up.unsqueeze(1)
    ...
    def convert_standard_pose_inverse(self, poses):
        """
        将 convert_standard_pose 后的 pose (N, 55, 3) 还原为标准顺序的 pose。
        """
        n = poses.shape[0]

        # 拆分各部分
        root_body = poses[:, :22]       # 0~21
        jaw = poses[:, 22:23]           # 22
        leye = poses[:, 23:24]          # 23
        reye = poses[:, 24:25]          # 24
        expr_lhand_rhand = poses[:, 25:]  # 25~54 (30 joints)

        # 重新拼接为标准 SMPL-X 顺序
        poses_std = torch.cat([
            root_body,            # 0~21
            expr_lhand_rhand,     # 22~51
            jaw
        ], dim=1)  # shape: (N, 53, 3)
