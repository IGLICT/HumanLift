...
class SMPLXModel(nn.Module):
    ...
    def get_neutral_pose_human(
        self, jaw_zero_pose, use_id_info, smplx_data, device, face_offset, joint_offset
    ):
        ...
        # 根关节归零：
        # 在角色动画或姿态估计中，根关节（如骨盆或脊柱基部）通常需要固定位置（偏移量为0），而其他关节（如四肢）可以自由偏移。
        # 例如：在SMPL/SMPLX人体模型中，根关节的偏移可能影响整体位移，需单独处理。
        if use_id_info:
    ...
    def transform_to_posed_verts_from_neutral_pose(
        ...
        print("nn_vertex",nn_vertex_idxs)
