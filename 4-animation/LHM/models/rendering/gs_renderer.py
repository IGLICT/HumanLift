...
from LHM.losses import Heuristic_ASAP_Loss,Heuristic_ACAP_Loss,LPIPSLoss,TVLoss,RestrictS_Loss
...
def readCameras(data_dir, K, R, T, orth = True):
    cam_infos = []
    num_views = 81
    ...
    for i in range(num_views):
        ...
        # print(T_i)
        # print(K_i)
        image_path = os.path.join(data_dir, filelist[i])
        # print(image_path)
        ...
        cam_infos.append(CameraTrain(uid=i, R=R_i, T=T_i, K=K_i,
                        image_name=image_name, image=image, mask=mask,
                        width=image.size[0], height=image.size[1]))
    return cam_infos

def readCamerasInfer(data_dir, K, R, T, scale = 1.0):
    cam_infos = []
    num_views = 81
    filelist = sorted(os.listdir(data_dir))
    bg = np.array([1,1,1])
    for i in range(num_views):
        ...
        # print(T_i)
        # print(K_i)
        K_i[0, 0] = K_i[0, 0]/scale
        K_i[1, 1] = K_i[1, 1]/scale
        image_path = os.path.join(data_dir, filelist[i])
        # print(image_path)
        image_name = image_path
        image = Image.open(image_path)
        ...
        image = Image.fromarray(np.array(arr*255.0, dtype=np.byte), "RGB")
        mask = Image.fromarray(np.array(np.repeat(norm_data[:, :, 3:4], 3, axis = 2)*255.0, dtype=np.byte), "RGB")
        # fx, fy = K_i[0, 0]/scale, K_i[1, 1]/scale
        cam_infos.append(CameraTrain(uid=i, R=R_i, T=T_i, K=K_i,
                        image_name=image_name, image=image, mask=mask,
                        width=image.size[0], height=image.size[1]))  
    return cam_infos
...
class CameraTrain:
    def __init__(
        self, uid,
        R, T, K, image, image_name, mask, width, height,
        ...
        if orth:
            ...
            self.FoVx = 2 * math.atan(image.size[0]/(2 * K[0,0]))
            self.FoVy = 2 * math.atan(image.size[1]/(2 * K[1,1]))
        self.height = height
        self.width = width
        ...
        if orth:
            self.projection_matrix = torch.tensor(K).to(self.world_view_transform.device)
            self.projection_matrix[2, 2] = -2.0 / (self.zfar - self.znear)
            self.projection_matrix[2, 3] = - (self.zfar + self.znear) / (self.zfar - self.znear)
            self.tanx = self.projection_matrix[0, 0]
            self.tany = self.projection_matrix[1, 1]
            self.projection_matrix = self.projection_matrix.transpose(0,1)
        else:
            self.projection_matrix = (
                ...
                .transpose(0, 1)
                .to(self.world_view_transform.device)
            )
        ...
        if mask is not None:
            resized_image_mask = PILtoTorch(mask)
            gt_mask = resized_image_mask[:1, ...] #(1,h,w)
        ...
        #     self.alpha_mask = mask[0:1, ...].to(self.data_device)
...
class GaussianModel:
    ...
    def __init__(self, xyz, opacity, rotation, scaling, shs, use_rgb=True) -> None:
        """
...
class GaussianModelTrain:
    ...
    # def __init__(self, offset_xyz, opacity, rotation, scaling, shs, use_rgb=True, random = False) -> None:
    #     """
    #     Initializes the GSRenderer object.
    #     Args:
    #         xyz (Tensor): The xyz coordinates.
    ...
    #     self._offset_xyz: Tensor = nn.Parameter(offset_xyz.requires_grad_(True))
    #     self._opacity: Tensor = nn.Parameter(inverse_sigmoid(opacity).requires_grad_(True))
    #     self._rotation: Tensor = nn.Parameter(rotation.requires_grad_(True))
    #     self._scaling: Tensor = nn.Parameter(torch.log(scaling).requires_grad_(True))
    #     self._shs: Tensor = nn.Parameter(shs.requires_grad_(True))  # [B, SH_Coeff, 3]
    #     self.optimizer = None
    ...
    def __init__(self, offset_xyz, opacity, rotation, scaling, shs, use_rgb=True, random=False) -> None:
        ...
        Args:
            ...
            shs (Tensor): The spherical harmonics coefficients.
            use_rgb (bool, optional): Indicates whether shs represents RGB values. Defaults to False.
            random (bool, optional): If True, initialize all parameters with random values. Defaults to False.
        """
        ...
        if random:
            # 随机初始化所有参数
            # 随机xyz偏移，使用正态分布
            self._offset_xyz = nn.Parameter(offset_xyz.requires_grad_(True))
            self._opacity = nn.Parameter(inverse_sigmoid(opacity).requires_grad_(True))
            self._rotation = nn.Parameter(rotation.requires_grad_(True))
            self._scaling = nn.Parameter(torch.log(scaling).requires_grad_(True))
            # 随机球谐系数
            self._shs = nn.Parameter(torch.randn_like(shs).requires_grad_(True))
        else:
            # 使用传入的参数
            self._offset_xyz = nn.Parameter(offset_xyz.requires_grad_(True))
            self._opacity = nn.Parameter(inverse_sigmoid(opacity).requires_grad_(True))
            self._rotation = nn.Parameter(rotation.requires_grad_(True))
            self._scaling = nn.Parameter(torch.log(scaling).requires_grad_(True))
            self._shs = nn.Parameter(shs.requires_grad_(True))  # [B, SH_Coeff, 3]
    ...
    def training_setup(self, frozen_indices = None, shs_indices = None):
        l = [
            {'params': [self._offset_xyz], 'lr': 0.00016, "name": "offset_xyz"}, #0.000016
            {'params': [self._shs], 'lr': 0.0025, "name": "shs"}, #0.0025
            {'params': [self._opacity], 'lr': 0.025, "name": "opacity"}, #0.025
            {'params': [self._scaling], 'lr': 0.005, "name": "scaling"}, #0.005
            {'params': [self._rotation], 'lr': 0.001, "name": "rotation"} #0.001
        ]
        self.optimizer = torch.optim.Adam(l, lr=0.0, eps=1e-15)
                # === 注册 hook 来冻结 frozen_indices ===
        # if frozen_indices is not None:
        #     frozen = torch.as_tensor(frozen_indices, device=self._offset_xyz.device)
        #     def freeze_hook(grad):
        #         grad = grad.clone()
        #         grad[frozen] = 0  # 直接把对应索引的梯度清零
        #         return grad
        #     self._offset_xyz.register_hook(freeze_hook)
        #     self._opacity.register_hook(freeze_hook)
        #     self._rotation.register_hook(freeze_hook)
        #     self._scaling.register_hook(freeze_hook)
        #     self._shs.register_hook(freeze_hook)
        # if shs_indices is not None:
        #     frozen = torch.as_tensor(shs_indices, device=self._offset_xyz.device)
        #     def freeze_shs_hook(grad):
        #         grad = grad.clone()
        #         grad[frozen] = 0  # 直接把对应索引的梯度清零
        #         return grad
        #     self._offset_xyz.register_hook(freeze_shs_hook)
        #     self._opacity.register_hook(freeze_shs_hook)
        #     self._rotation.register_hook(freeze_shs_hook)
        #     self._scaling.register_hook(freeze_shs_hook)
        frozen = torch.as_tensor(frozen_indices, device=self._offset_xyz.device)
        shs_mask = torch.as_tensor(shs_indices, device=self._offset_xyz.device)
        ...
        def make_hook(param_name):
            def hook(grad):
                ...
                if frozen is not None:
                    grad[frozen] = 0
                # 只保留颜色更新（其他参数清零）
                # if shs_mask is not None and param_name != "shs":
                #     grad[shs_mask] = 0
                if shs_mask is not None and param_name != "offset_xyz" and param_name != "scaling" and param_name != "rotation":
                    grad[shs_mask] = 0
                return grad
            return hook
        # 给每个参数都挂上 hook（颜色参数单独处理）
        self._offset_xyz.register_hook(make_hook("offset_xyz"))
        self._opacity.register_hook(make_hook("opacity"))
        self._rotation.register_hook(make_hook("rotation"))
        self._scaling.register_hook(make_hook("scaling"))
        self._shs.register_hook(make_hook("shs"))
    ...
    # def clone(self):
    #     xyz = self.offset_xyz.clone()+self.query_point.clone()
    #     opacity = self.opacity.clone()
    #     rotation = self.rotation.clone()
    #     scaling = self.scaling.clone()
    #     shs = self.shs.clone()
...
class GS3DRenderer(nn.Module):
    ...
    def forward_single_view_train(
        self,
        gs: GaussianModelTrain,
        viewpoint_camera: CameraTrain,
        background_color: Optional[Float[Tensor, "3"]],
        ret_mask: bool = True,
        orth = True,
    ):
        ...
        # Set up rasterization configuration
        if orth:
            tanfovx = viewpoint_camera.tanx
            tanfovy = viewpoint_camera.tany
        else:
            tanfovx = math.tan(viewpoint_camera.FoVx * 0.5)
            tanfovy = math.tan(viewpoint_camera.FoVy * 0.5)
        # print(tanfovx)
        ...
        with torch.autocast(device_type=self.device.type, dtype=torch.float32):
            ...
            render_colors, render_alphas, info = rasterization(
                ...
                packed=False,
                sh_degree=sh_degree,
                camera_model="ortho" if orth else "pinhole",
                render_mode="RGB+ED",
        ...
        ret = {
            "comp_rgb": render_colors[0,:,:,0:3],  # [H, W, 3]
            "comp_rgb_bg": bg_color,
            "comp_mask": render_alphas[0],
            "comp_depth": render_colors[0,:,:,3:4],
        }
        return ret
    def animate_gs_model(
        ...
        for cano_smplx_data_key in cano_smplx_data_keys:
            warp_data = smplx_data[cano_smplx_data_key] #[B,3] [B,N,3]
    ...
    def animate_gs_model_train(
            self, gsmodeltrain, query_points, smplx_data, debug=False
        ):
            """
            query_points: [N, 3]
            """

            device = gsmodeltrain.get_offset_xyz.device
            ...
            with torch.autocast(device_type=device.type, dtype=torch.float32):
                mean_3d = (
                    query_points + gsmodeltrain.get_offset_xyz
                )  # [N, 3]  # canonical space offset.
                ...
                is_constrain_body = self.smplx_model.is_constrain_body
                rigid_rotation_matrix[:, is_constrain_body] = I
                rotation_neutral_pose = gsmodeltrain.get_rotation.unsqueeze(0).repeat(num_view, 1, 1)
            ...
            for i in range(num_view):
                gs_copy = GaussianModel(
                    xyz=mean_3d[i],
                    opacity=gsmodeltrain.get_opacity,
                    rotation=rotation_pose_verts[i],
                    scaling=gsmodeltrain.get_scaling,
                    shs=gsmodeltrain.get_shs,
                    use_rgb=self.gs_net.use_rgb,
                )  # [N, 3]
    ...
    def get_query_points_zbt(self, smplx_data, device):
        print("zhixingget_query_points")
        with torch.no_grad():
            with torch.autocast(device_type=device.type, dtype=torch.float32):
                # print(smplx_data["betas"].shape, smplx_data["face_offset"].shape, smplx_data["joint_offset"].shape)
        ...
        return positions, smplx_data, positions_wosample
    ...
    def forward_single_batch_train(
        self,
        gs_list: list[GaussianModel],
        cam_info,
        background_color: Optional[Float[Tensor, "Nv 3"]],
        ...
        # for v_idx, (c2w, intrinsic) in enumerate(zip(c2ws, intrinsics)):
        #     out_list.append(
        #         self.forward_single_view_train(
        #             gs_list[v_idx],
        #             Camera.from_c2w(c2w, intrinsic, height, width),
        #             background_color[v_idx],
        #         )
    ...
    def forward_animate_gs_train(
        self,
        gs_attr_list, #gs_attr_list[0]
        ...
        #这里要写相机准备的代码
        print("相机准备")
        cam_infos = readCameras(dataset_dir,K,R,T)

        N_view = smplx_data["root_pose"].shape[1]
        print("N_view",N_view)
        gs_attr = gs_attr_list[0]
        query_pt = query_points[0]
        ...
        # len(animatable_gs_model_list) = num_view
        #这里设置优化参数和优化器
        gsmodeltrain = GaussianModelTrain(gs_attr.offset_xyz, gs_attr.opacity, gs_attr.rotation, gs_attr.scaling, gs_attr.shs,random = False)
        gsmodeltrain.training_setup(mask, shs_mask)

        lpips_loss_func = LPIPSLoss(device="cuda")
        ASAP_func = Heuristic_ASAP_Loss()
        ACAP_func = Heuristic_ACAP_Loss()
        tv_func = TVLoss()
        rs_func = RestrictS_Loss()

        N = gsmodeltrain.get_offset_xyz.shape[0]
        all_idx = torch.arange(N, device=gsmodeltrain.get_offset_xyz.device)
        rest_mask = all_idx[~torch.isin(all_idx, acap_mask)]

        #开始迭代训练

        for i in tqdm(range(3000)):
            merge_animatable_gs_model_list, cano_gs_model_list = self.animate_gs_model_train(
                gsmodeltrain,
                query_pt,
                self.get_single_batch_smpl_data(smplx_data, 0),
                debug=debug,
            )
            animatable_gs_model_list = merge_animatable_gs_model_list[:N_view]
            num = random.randint(0, 80)
            cam_info = cam_infos[num]
            out = self.forward_single_batch_train(
                        animatable_gs_model_list,
                        cam_info,
                        background_color[0] if background_color is not None else None,
                        debug=debug,
                    )
            image = out["comp_rgb"][0].permute(2, 0, 1)
            gt_image = cam_info.original_image.cuda()
            mask = out["comp_mask"][0].permute(2, 0, 1)
            gt_mask =  cam_info.original_mask.cuda()

            
            lpips_loss = lpips_loss_func((image * gt_mask).unsqueeze(0).unsqueeze(0),(gt_image * gt_mask).unsqueeze(0).unsqueeze(0))
            Ll1 = l1_loss(image* gt_mask, gt_image* gt_mask)
            mask_loss = l1_loss(mask, gt_mask)
            ssim_value = ssim(image* gt_mask, gt_image* gt_mask)


            
            ASAP_loss = 500 * ASAP_func(gsmodeltrain.get_scaling,r=8)
            if acap_mask is not None:
                ACAP_loss = 100 * (ACAP_func(gsmodeltrain.get_offset_xyz[acap_mask],d = 0.05625*0.3)+ACAP_func(gsmodeltrain.get_offset_xyz[rest_mask], d=0.05625*0.8))
            else:
                ACAP_loss = 100 * ACAP_func(gsmodeltrain.get_offset_xyz,d = 0.05625*0.5)  
            tv_loss = 0.01 * tv_func((image * gt_mask).unsqueeze(0).unsqueeze(0))        #100
            rs_loss =  50 * rs_func(gsmodeltrain.get_scaling, d=0.05625*0.5) #0.05625*0.5
            loss = 0.9 * Ll1 + 0.1 * lpips_loss + 0.5 * mask_loss + tv_loss + ACAP_loss  + ASAP_loss + rs_loss
            loss.backward()
            
            gsmodeltrain.optimizer.step()
            gsmodeltrain.optimizer.zero_grad(set_to_none = True)
        
        gs_attr.offset_xyz = gsmodeltrain.get_offset_xyz
        gs_attr.scaling = gsmodeltrain.get_scaling
        gs_attr.opacity = gsmodeltrain.get_opacity
        gs_attr.rotation = gsmodeltrain.get_rotation
        gs_attr.shs = gsmodeltrain.get_shs
        ...
        animatable_gs_model_list = merge_animatable_gs_model_list[:N_view]
        # animatable_gs_model_list = cano_gs_model_list[:N_view]
        animatable_gs_model = merge_animatable_gs_model_list[0]

        # print(len(animatable_gs_model_list))
        # print(len(cano_gs_model_list))
        assert len(animatable_gs_model_list) == c2w.shape[1]
        for cam_info in cam_infos:
            # gs render animated gs model.
            debug = True
            out_list.append(
                    self.forward_single_batch_train(
                        animatable_gs_model_list,
                        cam_info,
                        background_color[0] if background_color is not None else None,
                        debug=debug,
