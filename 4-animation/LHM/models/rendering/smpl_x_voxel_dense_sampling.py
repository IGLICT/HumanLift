...
class SMPLXVoxelMeshModel(nn.Module):
    def __init__(
        ...
        self.smplx_init()
        self.export_neutral_mesh()

    def export_neutral_mesh(self, out_path="./tmp/cano.obj", device=None):
        """
        生成标准姿势 (neutral pose) 的网格，并用 trimesh 导出保存.

        Args:
            out_path: 导出路径 (支持 .ply / .obj / .glb 等)
            device: torch.device, 默认和 smplx_layer 一致
        """
        # device = device if device is not None else self.smplx_layer.v_template.device
        device = device if device is not None else self.smplx_layer.v_template.device
        # 构造 smplx 参数 (标准姿势, 不带表情)
        smplx_data = {
            "betas": torch.zeros(1, self.smpl_x.shape_param_dim, device=device)
        }

        # 得到 neutral pose 顶点
        mesh_neutral_pose, mesh_neutral_pose_wo_sample, _ = self.get_neutral_pose_human(
            jaw_zero_pose=True,
            use_id_info=False,
            shape_param=smplx_data,
            device=device,
            face_offset=None,
            joint_offset=None,
        )

        verts = mesh_neutral_pose_wo_sample.squeeze(0).detach().cpu().numpy()  # (V,3)
        faces = self.smpl_x.face                   # (F,3)
        ...
        print(f"[SMPLX] Neutral pose mesh 已保存: {out_path}")
    ...
    def get_body_infos(self):
        ...
        verts = self.dense_pts.cuda()
        lower_z = verts[is_lower_body][:, 2]
        z_thresh = torch.quantile(lower_z, 0.15)  # 取最低 15% 的点
        is_feet = is_lower_body[lower_z <= z_thresh]

        # 左右脚可以通过 x 坐标划分
        feet_ids = torch.where(is_lower_body == True)[0]
        feet_verts = verts[feet_ids]
        left_foot = feet_ids[feet_verts[:, 0] < 0]
        right_foot = feet_ids[feet_verts[:, 0] > 0]
        # 眼睛：SMPL-X 提供 expr/eye mask，这里直接用 face_expr 近似
        eye_id = torch.where(self.is_face_expr == True)[0]
