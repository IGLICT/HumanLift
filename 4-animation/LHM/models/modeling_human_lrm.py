...
from LHM.models.rendering.gs_renderer import GS3DRenderer, PointEmbed, readCameras, readCamerasInfer
...
class ModelHumanLRM(nn.Module):
    ...
    def infer_single_view(
        ...
        query_points = None
        # print("self.latent_query_points_type",self.latent_query_points_type)
        if self.latent_query_points_type.startswith("e2e_smplx"):
    ...
    #     gs_model_list, query_points, smplx_params = self.renderer.forward_gs_zbt(
    #         gs_hidden_features=latent_points,
    #         query_points=query_points,
    #         smplx_data=smplx_params,
    #         additional_features={"image_feats": image_feats, "image": image[:, 0]},
    #     )
    #     print(gs_app_model_list)
    #     print(query_points)
    #     print(smplx_params)
...
class ModelHumanLRMSapdinoBodyHeadSD3_5(ModelHumanLRM):
    ...
    def infer_single_view(
        ...
        #得到大POSE的查询点和矩阵
            query_points, smplx_params, query_points_wosample = self.renderer.get_query_points_zbt(
                smplx_params, device=image.device
            )
            # print(smplx_params["transform_mat_neutral_pose"])
        ...
        # print(gs_model_list)
        return gs_model_list, query_points, query_points_wosample,smplx_params['transform_mat_neutral_pose']
    ...
    def animation_infer_train(self, gs_model_list, query_points, smplx_params, render_c2ws, render_intrs, render_bg_colors,dataset_dir,K,R,T, mask = None, shs_mask = None, new_params_list = [],acap_mask = None):
        '''Inference code avoid repeat forward.
        '''
        ...
        infer_cam_infos = readCamerasInfer(dataset_dir,K,R,T)
        def swing_camera_index(k, num_views=81, center=0, half_span=2):
            k = k//5
            cycle_len = 2 * half_span * 2  # 来回周期长度 (52)
            # 位置在一个周期内的索引
            pos = k % (2 * half_span * 2)  
            if pos <= 2 * half_span:
                offset = pos - half_span
            else:
                offset = (2 * half_span) - (pos - half_span)
            return (center + offset) % num_views
        ...
        with torch.no_grad():
            ...
            for new_params in new_params_list:
                ...
                render_res_list = []
                num_views = render_c2ws.shape[1]
                for idx in range(new_params["body_pose"].shape[1]):
                    new_idx = 0
                    render_res = self.renderer.forward_animate_gs_infer(
                            gs_model_list[0],
                            query_points[0],
                            self.renderer.get_single_view_smpl_data(new_params, idx),
                            render_c2ws[:, new_idx : new_idx + 1],
                            render_intrs[:, new_idx : new_idx + 1],
                            render_h,
                            render_w,
                            render_bg_colors[:, new_idx : new_idx + 1],
                            infer_cam_infos[0],
                        )
                    render_res_list.append(render_res)
                    cou = cou + 1
                
                infer_out = defaultdict(list)
                for res in render_res_list:
                    for k, v in res.items():
                        if isinstance(v[0], torch.Tensor):
                            infer_out[k].append(v.detach())
                        else:
                            infer_out[k].append(v)
                for k, v in infer_out.items():
                    # print(f"out key:{k}")
                    if isinstance(v[0], torch.Tensor):
                        infer_out[k] = torch.concat(v, dim=1)
                        if k in ["comp_rgb", "comp_mask", "comp_depth"]:
                            # print(infer_out[k].shape)
                            infer_out[k] = infer_out[k][0,:].permute(
                                0, 2, 3, 1
                            )  # [1, Nv, 3, H, W] -> [Nv, 3, H, W] - > [Nv, H, W, 3]
                    else:
                        infer_out[k] = v
                LHM_infer_out_list.append(infer_out)
                

        render_res_list = []
        for view_idx in range(num_views):
            render_res, gs_attr = self.renderer.forward_animate_gs_train(
                gs_model_list,
                query_points,
                self.renderer.get_single_view_smpl_data(smplx_params, view_idx),
                render_c2ws[:, view_idx : view_idx + 1],
        ...
        return out, infer_out_list, LHM_infer_out_list
