...
def prepare_motion_seqs(
    ...
    if ref_json_path is not None:
        with open(ref_json_path) as f:
            ref_data = json.load(f)
            trans_ref = torch.FloatTensor(ref_data["trans"])  # 参考trans
    ...
    for idx, smplx_path in enumerate(motion_seqs):
        ...
        if idx == 0:
            ...
            # 在这里调整trans
        if idx == 0:
            trans_first = smplx_param["trans"].clone()
            if trans_ref is not None:
                smplx_param["trans"] = trans_ref.clone()
        else:
            if trans_ref is not None:
                smplx_param["trans"] = trans_ref
                # smplx_param["trans"] = trans_ref + (smplx_param["trans"] - trans_first)
...
# 仅输入一个json
def prepare_motion_seqs_single_seq(
    motion_seqs_dir,
    image_folder,
    save_root,
    fps,
    ...
    motion_seqs = []
    for i in range(1):
        motion_seqs.append(motion_seqs_dir)
    # motion_seqs = sorted(glob.glob(os.path.join(motion_seqs_dir, "*.json")))
    # motion_seqs = motion_seqs[:motion_size]
